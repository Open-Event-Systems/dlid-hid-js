...
import { makeDLIDParser, type Header } from "./parse.js"
...
describe("parser", () => {
  test("parses test data", () => {
    const parser = makeDLIDParser("")
    const cont = parser.append(exampleData)
    expect(cont).toBe(false)

    expect(parser.error).toBeUndefined()
    expect(parser.done).toBe(true)
    expect(parser.header).toStrictEqual(exampleDataHeader)
    expect(parser.subfiles).toStrictEqual(exampleDataSubfiles)
  })

  test("parses test data (extra data separator)", () => {
    const parser = makeDLIDParser("")
    const cont = parser.append(exampleDataExtraSeparator)
    expect(cont).toBe(false)
  ...
  test("parses with multiple appends", () => {
    ...
    let cont = parser.append(exampleData.substring(0, 10))
    expect(cont).toBe(true)
    expect(parser.error).toBeUndefined()
    expect(parser.done).toBe(false)

    cont = parser.append(exampleData.substring(10, 100))
    expect(cont).toBe(true)
    expect(parser.error).toBeUndefined()
    expect(parser.done).toBe(false)

    cont = parser.append(exampleData.substring(100, 300))
    expect(cont).toBe(true)
    expect(parser.error).toBeUndefined()
    expect(parser.done).toBe(false)

    cont = parser.append(exampleData.substring(300))
    expect(cont).toBe(false)
  ...
  test("sets error", () => {
    ...
    parser.append("@  ANSI 000000000000XXXXXX")
    expect(parser.done).toBe(false)
    expect(parser.error).toBeInstanceOf(Error)
  })
})
